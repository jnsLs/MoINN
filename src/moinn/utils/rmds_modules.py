...
def read_xyz_file(filename, look_for_charge=True):
    ...
    charge = 0
    atomic_symbols = []  # ensemble
    xyz_coordinates = []  # ensemble
    with open(filename, "r") as file:
        for line_number, line in enumerate(file):
            if line_number == 0:
                num_atoms = int(line)
                num_lines_per_mol = num_atoms + 2
            elif line_number % num_lines_per_mol == 0:
                continue
            elif line_number % num_lines_per_mol == 1:
                mol_atomic_symbols = []
                mol_xyz_coordinates = []
            ##elif line_number > 100*num_lines_per_mol:
            ##    break
            else:
                atomic_symbol, x, y, z = line.split()
                mol_atomic_symbols.append(int(atomic_symbol))
                mol_xyz_coordinates.append([float(x), float(y), float(z)])
                # store in ensemble list
                if line_number % num_lines_per_mol == num_lines_per_mol - 1:
                    atomic_symbols.append(mol_atomic_symbols)
                    xyz_coordinates.append(mol_xyz_coordinates)

    # convert atomic number strings to integers
    # print(atomic_symbols[0][0])
    ##if type(atomic_symbols[0][0]) is str:
    ##    atoms = []
    ##    for asymbs in atomic_symbols:
    ##        atoms.append([int_atom(atom) for atom in asymbs])
    ##else:
...
def xyz2mol(atoms, coordinates,
    ...
    if AC is not None:
        _, mol = xyz2AC(atoms, coordinates, charge, use_huckel=use_huckel)
    else:
...
def convert(xyz_path=None, ensemble_atoms=None, ensemble_xyz_coordinates=None, take_each=10, con_path=None, con=None,
            traj_start=None, traj_end=None):
    """
    Function to convert xyz data to rdkit objects.

    Parameters
    ----------
    xyz_path : path to xyz file (e.g. trajectory)

    Returns rdkit mol object and number of conformers (int)
    -------
    ...
    # read atoms and coordinates (and connectivity matrix). Try to find the charge
    if xyz_path is not None:
        ensemble_atoms, charge, ensemble_xyz_coordinates = read_xyz_file(xyz_path)
        AC = None
    elif con_path is not None:
        charge = 0
        AC = pickle.load(open(con_path, "rb"))
        ...
        charge = 0
        AC = None
        #raise NotImplementedError('select either xyt_path, con_path or con')

    # Get the molobj and connectivity matrix
    mol, AC = xyz2mol(ensemble_atoms[0], ensemble_xyz_coordinates[0],
                      charge=charge,
                      use_graph=quick,
    ...
    if traj_end is not None:
        for idx in range(traj_start, traj_end):

            atoms = ensemble_atoms[idx]
            xyz_coordinates = ensemble_xyz_coordinates[idx]

            if idx % take_each == 0:
                mol_tmp, _ = xyz2mol(atoms, xyz_coordinates,
                                     charge=charge,
                                     use_graph=quick,
                                     allow_charged_fragments=charged_fragments,
                                     embed_chiral=embed_chiral,
                                     use_huckel=use_huckel,
                                     AC=AC)
                conf = mol_tmp.GetConformer(0)
                mol.AddConformer(conf, idx)
        n_confs = (traj_end - traj_start) // take_each
    else:
        for idx, (atoms, xyz_coordinates) in enumerate(zip(ensemble_atoms, ensemble_xyz_coordinates)):
            if idx % take_each == 0:
                mol_tmp, _ = xyz2mol(atoms, xyz_coordinates,
                                     charge=charge,
                                     use_graph=quick,
